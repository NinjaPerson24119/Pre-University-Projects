...
//Constructor
animation::animation(heapallocator *iAlloc):
alloc(iAlloc),
tempimgs(iAlloc),
temptimes(iAlloc),
imgs(iAlloc),
...
//Add frame
bool animation::add_frame(string &nImageFilename, float nTime)
{
	texture temp(alloc);
...
//Add frame
bool animation::add_frame(texture &nImg, float nTime)
{
	#ifdef CTM_DEBUG_SLOW
		if(DEFAULT_DEBUGLOG != 0)
			if(nImg.get_id() == 0)
			{
				DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to add frame! Image has not been loaded!\n",false);
				return false;
			}
	#endif
		...
		if(imgs.get_clipcount() != 0)
		{
			if(DEFAULT_DEBUGLOG != 0)
				...
				{
					DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to add frame! New frame dimensions are not equal to existing frame dimensions!\n",false);
					return false;
				}
		}
	#endif
...
//Add frames
bool animation::add_frames(vector1D<texture> &nImgs, vector1D<float> &nTimes)
{
	#ifdef CTM_DEBUG_SLOW
		if(DEFAULT_DEBUGLOG != 0)
			if(nImgs.get_size() != nTimes.get_size())
			{
				DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to add frames! Image and time vectors are not the same size!\n",false);
				return false;
			}
	#endif
		...
		if(DEFAULT_DEBUGLOG != 0)
		{
			for(unsigned int count = 0; count < nImgs.get_size(); ++count)
			{
				if((nImgs[0].get_imgwidth() != nImgs[count].get_imgwidth()) || (nImgs[0].get_imgheight() != nImgs[count].get_imgheight()))
				{
					DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to add frames! Frames do not all have equal dimensions!\n",false);
					return false;
				}
			}
		}
		...
		if(DEFAULT_DEBUGLOG != 0)
		{
			if(imgs.get_clipcount() != 0)
			{
				for(unsigned int count = 0; count < nImgs.get_size(); ++count)
				{
					...
					{
						DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to add frames! New frame dimensions are not equal to existing frame dimensions!\n",false);
						return false;
					}
				}
			}
		...
		if(DEFAULT_DEBUGLOG != 0)
		{
			for(unsigned int count = 0; count < nImgs.get_size(); ++count)
			{
				...
				{
					DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to add frames! One or more images have not been loaded!\n",false);
					return false;
				}
			}
		}
	...
	for(unsigned int count = 0; count < nTimes.get_size(); ++count)
	{
...
void animation::update_frames()
{
		...
		if(DEFAULT_DEBUGLOG != 0)
			if((tempimgs.get_size() == 0) || (temptimes.get_size() == 0))
				DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to update frames! No new frames exist!\n",false);
	#endif
	
	//Make bigger texture sheet
	texturesheet newsheet(alloc);
	...
	for(unsigned int count = 0; count < tempimgs.get_size(); ++count)
	{
	...
	//Add clips
	rect region(0,0,0,(float)tempimgs[0].get_imgheight());
	for(unsigned int count = 0; count < tempimgs.get_size()+imgs.get_clipcount(); ++count)
	{
		region.left = (float)(count*tempimgs[0].get_imgwidth());
		region.right = (float)(region.left + tempimgs[0].get_imgwidth());
	...
	for(unsigned int count = 0; count < temptimes.get_size(); ++count)
	{
...
void animation::update(float timepassed)
{
		...
		if(DEFAULT_DEBUGLOG != 0)
			if(imgs.get_clipcount() == 0)
				DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to update animation! No frames exist!\n",false);
	#endif
...
void animation::draw(float x, float y)
{
	...
	{
		if(DEFAULT_DEBUGLOG != 0)
			DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to draw animation! Animation has not been loaded!\n",true);
	}
	#endif
...
//Draw
void animation::draw(float x, float y, unsigned int frame)
{
	#ifdef CTM_DEBUG_SLOW
	if(frame >= imgs.get_clipcount())
	{
		if(DEFAULT_DEBUGLOG != 0)
			DEFAULT_DEBUGLOG->send("Error: cAnimation - Failed to draw animation! Invalid frame!\n",true);
	}
	#endif
