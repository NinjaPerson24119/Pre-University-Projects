...
template<class type> iterator<type> iterator<type>::operator++()
{
		...
		assert(!(current->next == 0)); //Error: cList - Failed to increment iterator! Next node does not exist!
	#endif
...
{
	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(current->previous == 0)); //Error: cList - Failed to decrement iterator! Previous node does not exist!
	#endif
...
{
	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(iter == get_tail())); //Error: cList - Cannot insert nodes after tail!
	#endif
...
{
	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(iter == get_head())); //Error: cList - Failed to erase element! Cannot erase head!
		assert(!(iter == get_tail())); //Error: cList - Failed to erase element! Cannot erase tail!
	#endif
