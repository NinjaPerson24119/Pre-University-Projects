...
{	
	...
	if(error != GL_NO_ERROR)
	{
			...
			if(DEFAULT_DEBUGLOG != 0)
				DEFAULT_DEBUGLOG->send_error("Error: cTexture - Failed to load texture from pixel data!\n",false);
		#endif
		return false;
	}
...
BOOL8 texture::load_texture(GLubyte *data, GLuint iTexWidth, GLuint iTexHeight, GLuint iImgWidth, GLuint iImgHeight)
{
	...
	GLuint *newdata = new GLuint[iTexWidth*iTexHeight]; //Allocate new RGBA data
	...
	delete[] newdata; //Deallocate RGBA data
	return flag;
}
...
BOOL8 texture::load_texture(string iFilename)
{
	...
	if(ilLoadImage(iFilename.get_char()) == false) //Check for errors
	{
		#ifdef CUSTOMLIB_DEBUG_FAST
			if(DEFAULT_DEBUGLOG != 0)
				DEFAULT_DEBUGLOG->send_error("Error: cTexture - Failed to load texture from file!\n",false);
		#endif
		return false;
	}
...
BOOL8 texture::load_texture(UINT32 iWidth, UINT32 iHeight)
{
	...
	GLuint *data = new GLuint[iWidth*iHeight]; //Allocate data
...
void texture::draw_texture(FLOAT32 x, FLOAT32 y)
{
	...
	else
	{
			...
			assert(0); //Error: cTexture - Failed to draw texture! Texture has not been loaded!
		#endif
	}
}
...
void texture::bind_texture()
{
	...
	{
		#ifdef CUSTOMLIB_DEBUG_FATAL
			assert(!(id == 0)); //Error: cTexture - Failed to bind texture! Texture has not been loaded!
			assert(!(binded == 1)); //Error: cTexture - Failed to bind texture ! Texture is already binded!
		#endif
	}
}
...
void texture::unbind_texture()
{
	...
	{
		#ifdef CUSTOMLIB_DEBUG_FATAL
			assert(!(id == 0)); //Error: cTexture - Failed to unbind texture! Texture is not binded!
		#endif
	}
}
...
{
	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(binded == 0)); //Error: cTexture - Failed to blit texture! Texture is not binded!
	#endif

	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(target.binded == 0)); //Error: cTexture - Failed to blit texture! Target texture is not binded!
	#endif

	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!((x+imgwidth > target.imgwidth) || (y+imgheight > target.imgheight))); //Error: cTexture - Failed to blit texture! Blitting coordinates are out of range!
	#endif
...
{
	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(binded == 0)); //Error: cTexture - Failed to pad texture! Texture is not binded!
	#endif
...
{
	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(binded == 0)); //Error: cTexture - Failed to get pixel! Texture is not binded!
	#endif

	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!((x > texwidth) || (y > texheight))); //Error: cTexture - Failed to get pixel! Coordinates are out of range!
	#endif
...
{
	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(binded == 0)); //Error: cTexture - Failed to set pixel! Texture is not binded!
	#endif

	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!((x >= texwidth) || (y >= texheight))); //Error: cTexture - Failed to set pixel! Coordinates are out of range!
	#endif
...
GLuint *pad_pixels(GLuint *data, GLuint &texwidth, GLuint &texheight, GLuint &imgwidth, GLuint &imgheight)
{
	#ifdef CUSTOMLIB_DEBUG_FATAL
		assert(!(data == 0)); //Error: cTexture - Failed to pad pixels! Invalid pointer to data!
	#endif
	...
	{
		GLuint *newdata = new GLuint[texwidth*texheight]; //Allocate new texture data
