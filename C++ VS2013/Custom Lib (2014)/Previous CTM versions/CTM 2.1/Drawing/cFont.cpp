...
BOOL8 font::load_font(string iFilename, UINT32 pixelsize)
{
	...
	FT_Error error = FT_Init_FreeType(&lib);
		...
		if(DEFAULT_DEBUGLOG != 0)
			if(error)
			{
				DEFAULT_DEBUGLOG->send("Error: cFont - Failed to initialize FreeType!\n",false);
				return false;
			}
	#endif
	...
	//Load face
	FT_Face face;
	error = FT_New_Face(lib,iFilename.get_cstr(),0,&face);

	#ifdef CUSTOMLIB_DEBUG_FAST
		if(DEFAULT_DEBUGLOG != 0)
			if(error)
			{
				DEFAULT_DEBUGLOG->send("Error: cFont - Failed to load face!\n",false);
	...
	error = FT_Set_Pixel_Sizes(face,pixelsize,pixelsize);
		...
		if(DEFAULT_DEBUGLOG != 0)
			if(error)
			{
				DEFAULT_DEBUGLOG->send("Error: cFont - Failed to scale face!\n",false);
				FT_Done_Face(face);
				FT_Done_FreeType(lib);
				return false;
			}
	...
	for(UINT32 count = 0; count < 256; ++count)
	{
		error = FT_Load_Char(face,count,FT_LOAD_RENDER); //Load next character
			...
			if(DEFAULT_DEBUGLOG)
				if(error)
				{
					DEFAULT_DEBUGLOG->send("Error: cFont - Failed to load character!\n",false);
					FT_Done_Face(face);
					FT_Done_FreeType(lib);
					return false;
				}
...
void font::draw_font(INT32 x, INT32 y)
{
	...
	{
		if(DEFAULT_DEBUGLOG != 0)
			DEFAULT_DEBUGLOG->send("Error: cFont - Failed to draw font! Font has not been loaded!\n",true);
		assert(0);
	}
	#endif
...
void font::text(string data)
{
	...
	{
		if(DEFAULT_DEBUGLOG != 0)
			DEFAULT_DEBUGLOG->send("Error: cFont - Failed to draw text! Font has not been loaded!\n",true);
		assert(0);
	}
	#endif
...
{
	move_pen(nX,nY); //Move pen
	text(data); //Write text normally
}

//Set cursor
void font::move_pen(INT32 nX, INT32 nY)
{
	x = nX;
	y = nY;
}
