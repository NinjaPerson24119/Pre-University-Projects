...
public:
	//System
		...
		vector1D(UINT32 iElements, type &iInitial, heapallocator *iAlloc);
	...
	//Memory
		...
		void resize(UINT32 nSize, const type &nInitial); //Resizes the vector
...
size(0),
capacity(0),
allowance(DEFAULT_VECTOR_ALLOWANCE)
{}
...
allowance(DEFAULT_VECTOR_ALLOWANCE)
{
	//Allocate data
	if(alloc == 0)
		data = (type*)new CHAR8[capacity*sizeof(type)];
	else
...
template<class type> void vector1D<type>::resize(UINT32 nSize)
{
	...
	type *copy = 0;
	if(alloc == 0)
		copy = (type*)new CHAR8[nSize*sizeof(type)];
	else
...
template<class type> void vector1D<type>::resize(UINT32 nSize, const type &nInitial)
{
	//Copy data
	type *copy = 0;
	if(alloc == 0)
